...
class LineConv(Module):
    ...
    def forward(self, item_embedding, D, A, session_item, session_len):
        ...
        seq_h1 = trans_to_cpu(torch.tensor([item.cpu().detach().numpy() for item in seq_h]))
...
class DHCN(Module):
    ...
    def init_parameters(self):
        ...
        # self.parameters().weight.data.eye_()
    ...
    def generate_sess_emb(self,item_embedding, session_item, session_len, reversed_sess_item, mask):
        ...
        zeros = torch.Tensor(torch.zeros(1, self.emb_size))
        item_embedding = torch.cat([zeros, torch.Tensor(item_embedding)], 0)
        # print("item_embedding:",item_embedding)
        get = lambda i: item_embedding[reversed_sess_item[i]]
        # print("reverse_session_item:",reversed_sess_item)
        # print('get:',get)
        # seq_h = torch.cuda.FloatTensor(self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size).fill_(0)
        seq_h = torch.zeros(self.batch_size, list(reversed_sess_item.shape)[1], self.emb_size)
        for i in torch.arange(session_item.shape[0]):
            seq_h[i] = get(i)
            # print("i,seq_h[i]:",i,seq_h[i])
        # print("seq_h:",seq_h)
    ...
    def forward(self, session_item, session_len, D, A, reversed_sess_item, mask):
        ...
        # print("item_embeddings_hg:",item_embeddings_hg)
...
def forward(model, i, data):
    ...
    scores = torch.mm(sess_emb_hgnn, torch.transpose(torch.Tensor(item_emb_hg), 1,0))
    return tar, scores, con_loss

def train_alone(model, train_data):
    print('start training: ', datetime.datetime.now())
    torch.autograd.set_detect_anomaly(True)
    total_loss = 0.0
    slices = train_data.generate_batch(model.batch_size)
    # print("model optimier:",model.optimizer)
    # print("slices len:",len(slices))
    for i in slices:
        model.zero_grad()
        targets, scores, con_loss = forward(model, i, train_data)
        # print("targets, scores, con_loss:",targets, scores, con_loss)
...
def test_alone(model, test_data, rs):
    ...
    # slices = slices[:int(rs*len(slices))]
    slices = random.sample(slices, int(rs*len(slices)))
